     ...
     (cond
       ...
       ((= address memory-control-begin)
        (when (/= value expansion-base-1-address)
          (error "Unexpected value for expansion-base-1-address, got 0x~8,'0x, expected 0x~8,'0x!~%" value expansion-base-1-address))
        value)
       ((= address (+ memory-control-begin 4))
        (when (/= value expansion-base-2-address)
          (error "Unexpected value for expansion-base-2-address, got 0x~8,'0x, expected 0x~8,'0x!~%" value expansion-base-2-address))
        value)
       ((= address (+ memory-control-begin #x10))
        (format t "Wrote 0x~8,'0x to bios delay/size!~%" value)
        value)
       (t (error "Unexpected write of 0x~8,'0x! to Memory Control at 0x~8,'0x!~%" value address) value)))
